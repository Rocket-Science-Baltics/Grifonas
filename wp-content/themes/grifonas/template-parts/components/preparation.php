...
$template_title = 'preparation';

if ($lang === 'lt-LT') {
    $template_title = 'pasiruosimas';
}
...
?>
<section id="<?php echo $template_title; ?>" class="pt-[54px] pb-[67px] bg-<?php echo $background_color; ?>">
    <div class="container">
        ...
        <div class="section-content">
            ...
            <div class="flex flex-col flex-wrap gap-x-[9px] gap-y-[9px]">
                <?php foreach ($list_section as $index => $list_item): ?>
                    <div class="flex items-center <?php echo ($list_item['list_paragraph']['paragraph_width'] !== '-') ? 'max-w-[' . $list_item['list_paragraph']['paragraph_width'] . ']' : 'w-full'; ?> <?php echo ($list_item['list_paragraph']['is_paragraph_heading']) ? 'mt-[34px] mb-[33px]' : ''; ?> gap-[19px]">
                        <?php
                        ...
                        if ($item_is_paragraph_heading) {
                            ...
                            echo '<div class="w-fit">' . wp_get_attachment_image($item_image_id, 'full', '', ['style' => 'max-width: -webkit-fill-available']) . '</div>';
                            echo $item_paragraph;
            ...
            <?php if (isset($additional_message)): ?>
                <?php echo '<div class="additional-message flex flex-col gap-[19px] items-start ' . (($additional_section_message['paragraph_width'] !== '-') ? 'max-w-[' . $additional_section_message['paragraph_width'] . ']' : 'w-full') . ' bg-' . $additional_message_bg . ' text-' . $additional_message_text_color . ' mt-[25px]">' . $additional_message . '</div>'; ?>
            <?php endif; ?>
        </div>
    </div>
</section>
